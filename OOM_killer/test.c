/* simple program to defragment memory, J. Cooperstein 2/042*/
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
#include <string.h>
...
#define CHUNK (MB*BS)
#define QUIT_TIME 20
void quit_on_timeout(intsig){   
    printf("\n\nTime expired, quitting\n");
    exit(EXIT_SUCCESS);
}

int main(int argc,char**argv){
    struct sysinfo si;int j, m;
    char *c;
    ...
    alarm(QUIT_TIME);
    for(j =0; j <= m; j += BS) {
        /* yes we know this is a memory leak, no free,45* that's the idea!46*/
        c = malloc(CHUNK);
